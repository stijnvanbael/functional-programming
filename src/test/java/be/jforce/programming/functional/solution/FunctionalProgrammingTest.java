package be.jforce.programming.functional.solution;

import be.jforce.programming.functional.Tweet;
import be.jforce.programming.functional.User;
import be.jforce.programming.functional.UserRepository;
 ...
 *         / \
 *        / | \    SPOILER ALERT!
 *       /  |  \   This is the solution, try solving the tests first
 *      /   .   \
 *      =========
 *
 */
...
public class FunctionalProgrammingTest {
    ...
    public void tweetsWithHashtagJava8() {
        List<Tweet> java8Tweets = tweets.stream()
                .filter(t -> t.hasHashtag("#Java8"))
                .collect(toList());
    ...
    public void tweetsForUser() {
        Map<User, List<Tweet>> tweetsForUser = tweets.stream()
                .collect(groupingBy(Tweet::getUser));
    ...
    public void referencedUsersByTweet() {
        ...
        Map<Tweet, List<User>> usersForTweet = tweets.parallelStream()
                .filter(t -> !t.getHandles().isEmpty())
                .collect(toMap((Tweet t) -> t, (Tweet t) -> t.getHandles().parallelStream()
                        .map(userRepository::byHandle)
                        .collect(toList())));
